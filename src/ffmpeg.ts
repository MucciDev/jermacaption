...
export const generateGIF = async (gif: string, identifier: string): Promise<Buffer> => {
  try {
    ...
    await sh(
        `ffmpeg -i "${textPath}" -i "${gifPath}" -i "${palettePath}" -filter_complex "[0:v]scale=700:-1[v0]; [1:v]scale=700:-1[v1]; [v0][v1]vstack=inputs=2,fps=15[v]; [v][2:v]paletteuse[v_optimized]" -map "[v_optimized]" -fs 9M -b:v 10M -loop 0 "${destinationPath}"`
    )
